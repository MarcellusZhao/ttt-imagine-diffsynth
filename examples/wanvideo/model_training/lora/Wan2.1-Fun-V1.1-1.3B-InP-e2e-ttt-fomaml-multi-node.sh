#!/usr/bin/env bash
# Multi-NODE (N nodes x 4 GPUs, DDP data-parallel) E2E-TTT meta-training for
# Wan2.1-Fun-V1.1-1.3B-InP on the i2v anchor route: one video per GPU per step, meta-gradients
# on phi_0 all-reduced across every rank on every node. All training knobs live in the YAML
# config; pass extra CLI flags after the config path to override individual values.
#
#   sbatch examples/wanvideo/model_training/lora/Wan2.1-Fun-V1.1-1.3B-InP-e2e-ttt-fomaml-multi-node.sbatch
...
# with srun). Use the single-GPU launcher ...-fomaml.sh for a 1-GPU job.
#
# Why a separate launcher: the single-GPU script calls `accelerate launch` directly, which
# spawns ranks on the LOCAL machine only. Multi-node needs one accelerate process per node
# (srun --ntasks-per-node=1), each told its own --machine_rank and a shared rendezvous
# endpoint. `--num_processes` is the GLOBAL rank count; accelerate divides it by
# --num_machines to get the per-node count. The h100 partition has 4 GPUs per node, so an
# 8-rank run is 2 nodes -- `--gres=gpu:8` is not satisfiable here.
#
# STEP COUNT CHANGES WITH RANK COUNT. accelerate shards the batch_size=1 loader round-robin,
# so one epoch is len(dataset) / num_processes optimizer steps: 3000 clips over 8 ranks is
# 375 steps, not 3000. Check that `save_steps` and `lr_warmup_steps` in the config still make
# sense at that scale, and raise `dataset_repeat` if 375 outer updates is too few to move
# phi_0 -- more GPUs make the SAME training faster, they do not add training.
#
...
if [ -z "${SLURM_JOB_ID}" ] || [ -z "${SLURM_JOB_NODELIST}" ]; then
    ...
    echo "       For a single GPU use Wan2.1-Fun-V1.1-1.3B-InP-e2e-ttt-fomaml.sh instead." >&2
    exit 1
fi
