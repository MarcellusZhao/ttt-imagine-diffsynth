#!/usr/bin/env bash
# Chunk-by-chunk VBench sampling with Wan2.1-T2V-1.3B, WITHOUT inter-chunk
# conditioning (--no_condition_on_last_chunk: every chunk is an independent
# text-to-video generation, concatenated), then print the per-dimension eval commands.
#
# Model / chunking / resolution come from the YAML config; this launcher only sets
# the per-pass sampling knobs and forces conditioning off via --no_condition_on_last_chunk
# (overriding the config's condition_on_last_chunk: true).
#
...
CONFIG="${CONFIG:-$SCRIPT_DIR/configs/Wan2.1-T2V-1.3B-chunk-by-chunk-vbench.yaml}"
SAVE_PATH="${SAVE_PATH:-/home/hzhao/ttt-imagine-diffsynth/results/vbench/Wan2.1-T2V-1.3B-chunk-by-chunk-without-conditioning}"

# 1) All dimensions: 1 video per prompt.
python "$SCRIPT_DIR/Wan2.1-T2V-1.3B-chunk-by-chunk-vbench.py" \
    --config "$CONFIG" \
    --dimension all \
    --save_path "$SAVE_PATH" \
    --num_videos_per_prompt 1 \
    --no_condition_on_last_chunk \
    --skip_existing
