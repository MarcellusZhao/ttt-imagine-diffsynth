#!/usr/bin/env bash
# Sample the full VBench T2V prompt suite with Wan2.1-T2V-1.3B, then print the
# per-dimension evaluation commands.
#
# Model weights, resolution and the umt5 tokenizer all come from the YAML config
# (configs/Wan2.1-T2V-1.3B-vbench.yaml); this launcher only sets the per-pass
# sampling knobs (dimension, videos-per-prompt) on the CLI, which override the YAML.
#
...
CONFIG="${CONFIG:-$SCRIPT_DIR/configs/Wan2.1-T2V-1.3B-vbench.yaml}"
SAVE_PATH="${SAVE_PATH:-/home/hzhao/ttt-imagine-diffsynth/results/vbench/Wan2.1-T2V-1.3B}"

# 1) All dimensions: 1 video per prompt.
python "$SCRIPT_DIR/Wan2.1-T2V-1.3B-vbench.py" \
    --config "$CONFIG" \
    --dimension all \
    --save_path "$SAVE_PATH" \
