...
On top of the LoRA memory, TI2V-5B can anchor each chunk on the previous chunk's last
frame via its fused VAE first-frame latent (`--condition_on_last_frame`, on by default);
the duplicated anchor frame at each boundary is dropped (`--drop_boundary_frame`, on by
default) to avoid a seam.
