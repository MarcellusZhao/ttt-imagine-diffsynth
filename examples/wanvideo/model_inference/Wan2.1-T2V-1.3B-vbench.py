"""
VBench prompt-suite sampling for Wan2.1-T2V-1.3B.
...
Because several VBench dimensions share the same prompt file
(`subject_consistency` is reused by `motion_smoothness`/`dynamic_degree`,
`scene` by `background_consistency`, `overall_consistency` by
`aesthetic_quality`/`imaging_quality`), sampling once from `all_dimension.txt`
into a single folder covers 15 of the 16 dimensions; only
`temporal_flickering` needs its extra indices topped up afterwards (the
...
# Dimensions whose prompt file is itself named differently from the dimension,
# plus the special 25-sample requirement for temporal_flickering, are handled
# by VBench at evaluation time; here we only need to know which .txt to read.
TEMPORAL_FLICKERING = "temporal_flickering"


def parse_args():
    parser = argparse.ArgumentParser(
        description="Sample the VBench prompt suite with Wan2.1-T2V-1.3B."
    )
    ...
    # Generation settings (Wan2.1-1.3B native 480p)
...
def main():
    ...
    print(f"[VBench/Wan2.1-T2V-1.3B] {len(shard)} prompts "
